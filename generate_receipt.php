...
if (!empty($bill_data['cashier_id'])) {
    ...
    if ($cashier_result && mysqli_num_rows($cashier_result) > 0) {
        $cashier_data = mysqli_fetch_assoc($cashier_result);
        $cashier_name = $cashier_data['fname'] . ' ' . $cashier_data['lname'];
        $pdf->SetFont('helvetica', 'B', 10);
        $pdf->Cell(40, 6, 'Processed By:', 0, 0);
        $pdf->SetFont('helvetica', '', 10);
        $pdf->Cell(0, 6, $cashier_name, 0, 1);
    }
}
