...
// Define Peso sign - using UTF-8 character
$peso_sign = "₱";
...
$pdf->SetFillColor(240, 248, 255);
$pdf->SetFont('dejavusans', 'B', 14);
$pdf->Cell(0, 10, 'Bill Breakdown', 1, 1, 'L', true);
$pdf->SetFont('dejavusans', '', 12);
$pdf->SetFillColor(255, 255, 255); 
$items = [
...
foreach ($items as $item) {
    if ($item[1] > 0) {
        ...
        $pdf->Cell(0, 8, $peso_sign . number_format($item[1], 2), 0, 1, 'R');
    }
}
...
if ($calculated_medicine_fees > 0) {
    ...
    $pdf->Cell(0, 8, 'Medicine Details:', 1, 1, 'L', true);
    $pdf->SetFont('dejavusans', '', 10);
    $pdf->SetFillColor(255, 255, 255);
    $medicine_details_query = "SELECT prescribed_medicines, dosage, frequency, duration, price FROM prestb WHERE pid = '$pid' AND diagnosis_details IS NOT NULL AND diagnosis_details != ''";
    $medicine_details_result = mysqli_query($con, $medicine_details_query);
    $pdf->SetFont('dejavusans', 'B', 9);
    $pdf->Cell(40, 7, 'Medicine', 1, 0, 'C', true);
    $pdf->Cell(25, 7, 'Dosage', 1, 0, 'C', true);
    $pdf->Cell(30, 7, 'Frequency', 1, 0, 'C', true);
    $pdf->Cell(25, 7, 'Duration', 1, 0, 'C', true);
    $pdf->Cell(20, 7, 'Price', 1, 1, 'C', true);
    ...
    while ($med = mysqli_fetch_assoc($medicine_details_result)) {
        ...
        for ($i = 0; $i < count($medicines); $i++) {
            ...
            $price = ($i == 0) ? $peso_sign . number_format($med['price'], 2) : ''; // Show price only once
            $pdf->Cell(40, 6, $med_name, 1, 0, 'L');
            $pdf->Cell(25, 6, $dosage, 1, 0, 'L');
            $pdf->Cell(30, 6, $freq, 1, 0, 'L');
            $pdf->Cell(25, 6, $dur, 1, 0, 'L');
            $pdf->Cell(20, 6, $price, 1, 1, 'R');
        }
    }
}
...
if ($bill_data['lab_fees'] > 0) {
    ...
    while ($lab = mysqli_fetch_assoc($lab_details_result)) {
        ...
        $pdf->Cell(0, 6, $peso_sign . number_format($lab['price'], 2), 0, 1, 'R');
    }
}
...
if ($bill_data['service_charges'] > 0) {
    ...
    while ($service = mysqli_fetch_assoc($service_details_result)) {
        ...
        $pdf->Cell(0, 6, $peso_sign . number_format($service['total_price'], 2), 0, 1, 'R');
    }
}
...
$pdf->Cell(0, 8, $peso_sign . number_format($calculated_total, 2), 0, 1, 'R');

if (($bill_data['discount'] ?? 0) > 0) {
    $pdf->Cell(120, 8, 'Discount:', 0, 0);
    $pdf->Cell(0, 8, '-' . $peso_sign . number_format($bill_data['discount'], 2), 0, 1, 'R');
}

$pdf->SetFont('dejavusans', 'B', 14);
$pdf->Cell(120, 10, 'TOTAL AMOUNT PAID:', 0, 0);
$pdf->Cell(0, 10, $peso_sign . number_format($calculated_total - ($bill_data['discount'] ?? 0), 2), 0, 1, 'R');
